// vectorizedMem_2DThreadTiling here improved on top of smem_2DThreadTiling earlier by vectorized memory access
...
//
// to vectorized loads/stores from/to GMEM, to use this to promise the compiler on 128b alignment (*):
// reinterpret_cast<float4 *>
// (*) but not used below at this point
//
// note that:
// we need to make sure the data alignment is a multiple of 16 bytes (128b)
// this is why we will have to pad the leading dimension of the matrix A and matrix B
...
1) nvcc cuda_sgemm_vectorizedMem_2DThreadTiling.cu -o cuda_sgemm_vectorizedMem_2DThreadTiling

2) .\cuda_sgemm_vectorizedMem_2DThreadTiling

3) ncu -o profile cuda_sgemm_vectorizedMem_2DThreadTiling.exe
...
*/
__global__ void sgemm_vectorizedMem_2DThreadTiling(int M, int N, int K, float alpha, const float* A,
    const float* B, float beta, float* C) {
...
//more inputs - end
    ...
    sgemm_vectorizedMem_2DThreadTiling<<<gridDim, blockDim>>>(m, n, k, alpha, d_A, d_B, beta, d_C);
