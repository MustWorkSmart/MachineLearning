// globalMemCoalescedWithinWarp here as compared to naive sgemm earlier (*)
// let's see how much slower than cublas (cublas_sgemm_alpha1_beta0.cu) and how much faster than cuda_sgemm_naive.cu
...
// (*)
// basically, since neighboring threadIdx.x would be in the same warp, but former setup got such down a given column crossing rows
// with the matrices in row-major-order, we need to have neighboring items in a row being threads within the same warp (which has 32 threads)
// hence, swapping the assignments for x/y in the kernel, and swapping M/N in the gridDim accordingly

/*
steps:

1) nvcc cuda_sgemm_globalMemCoalescedWithinWarp.cu -o cuda_sgemm_globalMem

2) .\cuda_sgemm_globalMem

m is 16384 .. k is 16384 .. n is 16384
Init took 10.142000 seconds.  Begin compute
Done. Compute took 12.937000 seconds
...
3) ncu -o profile cuda_sgemm_globalMem.exe
...
//more inputs - end
    ...
    sgemm_globalMem<<<gridDim, blockDim>>>(m, n, k, alpha, d_A, d_B, beta, d_C);
